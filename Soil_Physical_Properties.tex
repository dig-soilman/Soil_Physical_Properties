...
\usepackage[a5paper, parfill, inner=1.5cm, outer=1.5cm, top=1.75cm, bottom=1.5cm]{geometry} % this will be half an A4 and is better presented on smartphones
...
% the following Structure size class table does not translate to html via pandoc well. Consider revising or presenting in another form.
...
Bulk Density $\left(\rho_b\right)$ is a measure of the weight of the soil per unit volume . Volume includes both solids and pore space. Usually reported as the weight of oven-dry soil following heating to 110\textdegree C.
...
\begin{itemize}
    \item Particle density; $\rho_s$
    \item Texture because it affects total porosity $\left(\eta\right)$:  $\text{sand}\,\rho_{\text{b}} > \text{clay}\,\rho_{\text{b}}$
    \item Soil structure because it affects porosity $\left(\eta\right)$; $\uparrow$  structure generally $\uparrow\,\eta$
...
porosity varies with texture, structure, and bulk density. Soils with more clay tend to have greater porosity than soils where sands dominate the size separates. The pores that arise owing to the size and irregular shape of the soils size separates are called \textbf{matrix pores}. The aggregation of the size separates that favor the repeating planes of separation between the peds and the orientation of the aggregates that result in the larger pores. The resulting repeating planes of separation tend to increase porosity. These pores between peds together with burrowing arthropods, abandoned root channels, and voids caused by entrapped gasses by water movement are called \textbf{nonmatrix pores}. nonmatrix pores are generally larger in cross section than matrix pores.
