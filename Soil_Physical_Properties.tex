...
It is common, frequent, and convenient to speak more broadly, generally, or more inclusively about classes of soil texture. Commonly employed is referring to soil materials as \textit{General texture groupings} of \textbf{sandy}, \textbf{loamy}, or \textbf{clayey}. Leveraging these General texture grouping in discussions the way in which these three terms are used determines if the term is a General texture grouping or a textural category from the twelve basic textural classes. This can initially be confusing to the uninitiated so when it doubt, ask, because a \textit{sandy, loamy, or clayey} soil materials reference is not the same as a \textit{sand, loam, or clay} textural class. Further, the \textit{General texture groups} are frequently referred to as five \textit{General texture subgroups}.

As mentioned previously High School FFA Land Judging teams determine a textural class from the five category system. A modified decision tree is presented in Figure \ref{fig:TextureByFeelFlowChart5class}.
...
When the term \csname{Clayey}\endcsname is employed it is not a textural class term. It is confusing to new learners when the class terms such as clayey is used, but the term is employed widely and thus this presentation this presentation.

 For instance high-school FFA Land Judgers learn and employ the five \textit{General texture subgroups} wherein \textbf{\textit{Fine}} is the correct textural class for the twelve class categories \textit{Clay}, \textit{Sandy Clay}, and \textit{Silty Clay} and this would be the same \textbf{\textit{Clayey}} from the three category textural class system. Similarly, the \textit{Sand} and \textit{Loamy Sand} classes from the twelve basic category system become \textbf{\textit{coarse}} from the General texture subcategory, or \textbf{\textit{Sandy}} from the General texture category. Figure \ref{fig:TextureClass12-5-3} depicts these relationships graphically.

 \begin{figure*}
    \includegraphics[width=0.9\columnwidth]{images/Textural_Classes_3-systems_bw-25pct-72pxpin-1013x1150px.png}
    \caption{Diagram relating the twelve basic textural classes, the five General texture groups and the three General texture subgroups. The High School organization FFA uses the USDA five  general texture subgroups for their Carrier Development Events.}
