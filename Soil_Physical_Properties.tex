...
% ToDo dig-soilman: add a passage to introduce not only the 12 category system, but also the 5, and 3 category system.
...
% ToDo dig-soilman: Find a TiKz coded rendering of the Gibbs triangle/ Textural Triangle with 12 classes

% ToDo dig-soilman: Find a TiKz coded rendering of the Gibbs triangle/textural triangle with 5 classes
...
The field method of determining textural class is the \enquote{feel method} where sand feels gritty, silt feels smooth or floury, and clay feels sticky. The subsequently determined proportions are classed and as \enquote{apparent texture}.
...
The five textural class system and a three textural class system are leveraged within the US Soil Taxonomy system and in the way soils are presented in the numerous technical classification systems e.g. suitability and limitations. When the term \csname{Clayey}\endcsname is employed it can be a textural  class term. It is confusing to new learners when the class terms such as clayey is used, but not previously presented or if presented in an introductory soil coarse not fully appreciated. Additionally, high-school FFA Land Judgers learn the five class system wherein \textbf{\textit{Fine}} is the correct textural class for the twelve class categories \textit{Clay}, \textit{Sandy Clay}, and \textit{Silty Clay} and this would be the same \textbf{\textit{Clayey}} from the three category textural class system. Similarly, the \textit{Sand} and \textit{Loamy Sand} classes from the twelve category system become \textbf{\textit{coarse}} or \textbf{\textit{Sandy}} in the five and three category system respectively Figure \ref{fig:TextureByFeelFlowChart5class}.
...
The clay and sand are also subdivided. Memorizing the sand fractions is particularly useful since these sizes often show up in names of soil map units. Soil map units convey information about patterns of spatial variability across the landscape. Additionally, in more advanced coarses students are typically asked to be responsible for the size limits of the fine silt and coarse silt, and occasionally even the clay which is fractionated into:
