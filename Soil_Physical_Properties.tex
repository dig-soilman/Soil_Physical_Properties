...
\usepackage{svg} % allows use of svg graphics
...
% \usepackage[toc]{appendix} % un-comment if using TOC and want appendix items to show in TOC
\usepackage{tabularray} % provides a bit more space \begin{tblr}
...
%===========
%+Thanks
%Thanks to Shaline Hai-Jew shalin@ksu.edu for the alert that in beta0.7.2 that some text boxes did not have title case.
% 
%-Thanks
%===========

%+Title
  \title{\Huge\bf\ Soil Physical Properties\\ \, \\\normalsize Version: Beta 0.7.2.4}
...
McGahan, D. G. (2022). Soil Physical Properties (beta 0.7.2.3). \href{https://github.com/dig-soilman/Soil_Physical_Properties}{https://github.com/dig-soilman/Soil\_Physical\_Properties}.
...
LaTeX source code at \href{https://github.com/dig-soilman/Soil_Physical_Properties.git}{https://gihttps://thub.com/dig-soilman/Soil\_Physical\_Properties.git}.

%=Un-comment the below if the TOC or lists are desired=
%\tableofcontents 
%\listoffigures
%\listoftables
...
Initial recordings of physical properties are to be just that, an assessment of the observation and a recording. But record what? Each property has been assigned a sensible scalar. Frequently, the method to determine what is recorded can sometimes be an actual measurement but can be that of a categorical class agreed upon by users at large. Different groups may leverage different categorical classes. 
...
% TODO dig-soilman: Effervescence needs a little more attention but would be more fully explored in a Soil Water Chapter (Could call a treatise with suggested title Soil Climate and do temperature in the same chapter with water.) As of September 2022 the 'Soil_Climate' treatise has been initiated by dig-soilman in GitHub. Making it pubic has not been initiated.
%=======================================
...
It is noteworthy to state that mineral size separates are  based on only the mineral fraction. Organics, salts, and evaporites are not included as part of the whole in determination and reporting any given size separate classing.
...
\begin{tblr}{|lc|}
\hline
Sand Separate    & Diameter (mm) \\ \hline 
...
\begin{table}[!htbp]
\centering
\caption{Table of coarse fraction (CF) content volume percentages and modifier names.}
...
\begin{tblr}{ m{12em} m{5.5em} m{5.5em} m{5.5em} } \\[0.75em]
...
It can be challenging to determine the $V_v$ when the relative proportions of $V_a$ and $V_w$ are unknown. When $V_a = 0$ the soil is saturated. Saturating a known total volume $\left(V_t\right)$  such as a core extracted from a soil and massing the saturated soil is useful because the density of water, its specific gravity, is know to be \qty[per-mode = symbol]{1}{\gram\per\cubic\centi\metre}. Since $V_a=0$ then $V_w=V_v$. The same sample can then have the water driven off by heating in a \qty{110}{\degreeCelsius} oven until it no longer reduces in mass and the difference in mass equates to a volume of voids $\left(V_v\right)$ that the water occupied. This is useful because the particle density does not have to be measured.
 ...
 \begin{figure}
     ...
     \caption{Illustration of "idealized" soil volume (V) occupied by voids and solids (s). The void volume is divided between air (a) and water (w). The mass (M) relationship is dominated by solids due to there greater density. Air  contributes very little mass compared to water and solids. t is total and p is pores. Figure: Donald G. McGahan Creative Commons Attribution-ShareAlike-4.0-International.}
...
\begin{tblr}{ll}
\hline
Class term             & pH range         \\ \hline %\hhline{==}
...
\begin{tblr}{ll}
\hline
Effervescence Class         & Criteria                          \\ \hline %\hhline{==}
